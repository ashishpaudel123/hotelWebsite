...
export class AuthService {
  ...
  async login(dto: LoginDto): Promise<AuthResponseDto> {
    ...
    if (user.lockUntil && user.lockUntil > new Date()) {
      ...
      throw new AppError('Account temporarily locked. Try again later.', 403, 'AUTH_003');
    }
  ...
  async refreshToken(refreshToken: string): Promise<AuthResponseDto> {
    ...
    try {
      payload = jwt.verify(refreshToken, this.REFRESH_TOKEN_SECRET) as TokenPayloadDto;
    } catch (error) {
  ...
  private generateAccessToken(userId: string, email: string, role: string, permissions: string[]): string {
