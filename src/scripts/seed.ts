...
import { RoomType, Room, WebsiteSettings, HomepageSection } from '../models';
...
const runSeed = async () => {
  try {
    ...
    const standardRoomType = await RoomType.create({
      name: 'Standard Room',
      slug: 'standard-room',
    ...
    const suiteRoomType = await RoomType.create({
    ...
    const presidentialRoomType = await RoomType.create({
    ...
    for (let i = 0; i < roomNumbers.length; i++) {
      let roomType;
      if (i < 3) roomType = standardRoomType._id;
      else if (i < 6) roomType = deluxeRoomType._id;
      else if (i < 9) roomType = suiteRoomType._id;
      else roomType = presidentialRoomType._id;
    ...
    const lunchCategory = await MenuCategory.create({
      name: 'Lunch',
      slug: 'lunch',
