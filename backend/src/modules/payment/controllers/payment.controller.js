...
router.post('/initiate', 
  ...
  async (req, res, next) => {
    try {
      ...
      let result;
      if (provider === 'esewa') {
        result = await paymentService.initESewaPayment(bookingId, booking.pricing.total);
      } else if (provider === 'khalti') {
        result = await paymentService.initKhaltiPayment(bookingId, booking.pricing.total);
      } else {
        return res.status(400).json({
          success: false,
          error: {
...
router.post('/webhook/esewa', 
  async (req, res, next) => {
    try {
      ...
      const result = await paymentService.verifyESewaPayment(data.transaction_uuid, data);
...
router.post('/webhook/khalti', 
  async (req, res, next) => {
    try {
      ...
      const result = await paymentService.verifyKhaltiPayment(transaction_id, pidx);
